...
from ...utils.dpt_utils import get_readout_oper, Transpose
...
class DPTdept(nn.Module):
    def __init__(self,backbone,features,readout,
        channels_last=False,
        use_bn=False,
    ...
    def _register_hooks_and_add_postprocess(self,
                        size=(384, 384),
    ...
    def _make_scratch(self,in_shape, out_shape:int , groups=1, expand=False):
        ...
        for i in range(4):
            layer = nn.Conv2d(
                in_shape[i],
                out_shape[i] *(2)**(i) if expand else out_shape[i],
                kernel_size=3,
                stride=1,
                padding=1,
                bias=False,
